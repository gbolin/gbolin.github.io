...
>**⚠️注**: *最后更新* 列表根据文章的 git 修改记录生成, 所以运行前先把 `_posts` 目录的修改提交.
...
另外，如果你使用 Project 类型, 修改配置文件的 `baseurl` 为项目名称, 以斜杠开头，如：`/project`。
...
生成的静态文件将会在 `/path/to/local/<new-repo>`. 把新仓库的修改提交并推送到远端 `master` 分支.
